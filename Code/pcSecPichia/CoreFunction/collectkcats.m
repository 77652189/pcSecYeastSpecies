...
org_name = 'komagataella pastoris';
...
% collect enzyme data
enzyme_list = model.mets(contains(model.mets,'_complex'));
...
%enzymedata.proteins = strrep(enzymedata.proteins,'_','-');Why is this here?! % all protein
...
for i = 1:length(enzyme_list)
    ...
    s_tmp = model.S(:,idx_tmp);
    mets_tmp = model.metNames(s_tmp < 0);%-值的为substrate
