...
model = changeRxnBounds(model,'BIOMASS',mu);
...
for i = 1:length(dil_rxns)
    ...
    if isempty(MW)
     ...
     if isempty(MW)
        MW = enzymedata.proteinMWs(contains(enzymedata.proteins,extractBefore(comp_name,'_misfolding')));
    end
	coeff = MW(1)/1000;
	if i == 1
...
eq = sprintf('%s + %.15f X%d%c',eq,40000/1000,idx,sep); %40000 is MW of dummy complex (g/mol)%%%%%%%

% dummy complex
idx = find(strcmp(model.rxns,'dilute_dummyER'));
eq = sprintf('%s + %.15f X%d%c',eq,40000/1000,idx,sep); %40000 is MW of dummy complex (g/mol)%%%%%%%

fprintf(fptr,'CM%d: %s = %.15f\n',k1+1,eq,mu*f); % unmodeled part in the fraction eliminate only the unmodeled protein part in the biomass equation

%unmodeled ER protein
fprintf(fptr,'CM%d: %.15f X%d = %.15f\n',k1+2,40000/1000,idx,mu*funmodelER);
