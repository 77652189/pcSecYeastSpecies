...
**真实工艺 μ 验证（机制层）**：在手发酵罐验证核验了 hLF 两相 μ——甘油生长相 μ≈0.10（跨温度 20–30℃ / pH 4–7 稳健）、葡萄糖生产相实测慢（限量补料生产相；数据里的低值是甘油长满后高密度尾段，非葡萄糖生长能力）。结论：**模型默认 μ=0.10 与 hLF 甘油生长相实测一致**，确认默认合理。生长速率取**生长相**值；葡萄糖生产相虽慢属工艺操作点，按工艺决定不作模型 μ（模型统一用默认 μ=0.10）。原始 OD / 菌株 / 温度·pH 明细为机密，仅本地私有区、不入 git；titer 锚点仍缺。
