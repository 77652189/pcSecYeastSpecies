...
> **本文件只放「实现结构变了」才需要改的内容**：分层边界、模块所有权、能力清单、
> 数据与产物治理、测试网关。
>
> 原则:**写边界，不写结构图。** 目录树与模块清单能从代码读出来，写在这里只会分叉；
> 只有"为什么这层不许碰那层"这类推导不出来的约束才留在这。
>
> 要什么、能力边界到哪 → [需求与能力边界](requirements.md)。
> 当前进展 → [执行计划](EXECUTION_PLAN.md)。**本文件不持有当前状态的权威。**
...
reaction proxy、relative uncalibrated、absolute unavailable 和 not executable 由核心层统一判定；绝对 gene-capacity 因缺审核 baseline capacity 保持不可执行。这是**分层规则**，不随进度变。

各方向进展到哪一步、哪些已验收、当前卡在什么地方，见 [执行计划](EXECUTION_PLAN.md)——本文件不重复陈述，否则两处状态必然分叉。
