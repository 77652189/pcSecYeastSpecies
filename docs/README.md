...
当前系统已经具备 KO/OE 候选生成、证据复核、实验反馈回放、gene-enzyme-reaction capacity 求解，以及外部 baseline capacity 候选的人工导入、审核和显式提升链路，但不能承诺真实发酵产量、mg/L 绝对值或实验成功率。Round 6A A0c 已完成 ecPichia G6PDH2 provenance closure，结论为 `architecture_decision_required`：现有证据不能形成可审核的绝对 baseline capacity，决策前停止增加同类来源适配器且不进入 Round 6B。
