...
- [x] **D3 分层短名单编排（L1/L2）**（本地，待 push）`app/services/per_strain_shortlist_run.py`：**L1** `build_modified_strain_shortlist`——读 D4 基线短名单（OE+KO 各 ratio>1 top-N）+ 野生型&改造后各一次瓶颈（C2）+ D2 打标；未命中基线 → 诚实 `needs_baseline_build`。**L2** `recompute_stale_candidates` + 引擎 `next_oe_candidates.recompute_modified_strain_candidate_effects`——只对**已失效**候选在改造后菌株重算（OE 复用 R2 sweep 取 `max_relative_gain`、KO 复用 D1 `run_knockout_screen` 取 `delta/改造后基线`）+ 与复用值合并重排；重算不可行 → `recompute_failed` 回退野生型值显式标。7 单测（真 D4 读+真 D2 打标、mock C2/L2 求解）+ L2 引擎真跑 smoke 验过（OE sweep + KO screen 出效应、0 warning）。
- [x] **D5 UI 整合**（本地，待 push）`simulation_results.py::_render_modified_strain_shortlist`——复用 C3 的 `pichia_last_run_modifications` 暂存；按钮 opt-in 构建 L1（OE+KO 短名单 + 分泌层 + 复用/失效徽标 + 相对效应）→ 已失效项按钮触发 L2 重算；未命中基线诚实指引先跑后台构建。import 校验通过；populated 路径全渲染需先有基线（D6/用户择机跑）。
- [x] **D6 测试 + 真实验证 + 文档**（本地，待 push）：路2 live demo——ingest 既有 `overnight_hLF_full` CSV（2026-07-03·pre-current-code·source_run 标清 freshness caveat）为真基线，真跑 D5 背后的 L1→L2（real solves）：L1 `available=True`、读回 1025 OE+1025 KO、瓶颈+打标端到端通；app 启动含 D5 面板无崩、读路径 `available=True`。**两处实跑发现并修**：① L1 只需瓶颈层却跑了带 top-N sweep 的全量 analyze（~50 冗余求解）→ 传 `top_n=0` 跳过，L1 从 ~50 解降 ~2 解（面板按钮从 ~10min 降 ~1min）；② 效应显示 `:.3g` 有效数字（本模型效应亚百分比、固定 2 位全 0.00%）。**诚实发现（记牢）**：约束档关的 overnight 基线下**瓶颈弥散于所有层** → union 口径下全部候选 `已失效`（保守正确、但复用无增益）；**复用增益只在瓶颈集中（约束档开、folding-limited）时兑现**（那时非折叠层 slack→可复用）——路2 demo 因基线是约束关只验机制、未展示增益，真增益待约束档开的新鲜基线。全量回归 598+355 在修前提交点绿、修后靶向 7+import 绿（改动为 perf/显示）。
