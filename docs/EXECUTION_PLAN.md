...
- [x] **E1 候选面板诚实化**：暴露策展俗名 + 分泌环节 + 可跑的复合体反应三列；“仅复核/模型外”改为“只能按反应跑”；新增独立按钮把复合体反应填进反应框（与“可执行”按钮分开，保留引擎的 executable / review 边界）。
- [x] **E0 前置易用性**：筛查页基因显示可读名（位点号精确匹配档 + 诚实标注）、KO 候选可视化（与 OE 对等 + 分泌↔生长权衡）、「基因扰动」信息层级重排。
...
- **显示层单一入口**：反应名经 `display_naming` 统一渲染（图表不再只显示反应 id）；枚举释义收拢进中央字典 + 按域覆盖，消除重复定义与同码多义。伴随的崩溃修复与守卫加固不在此列——那是 `git log` 的事。
