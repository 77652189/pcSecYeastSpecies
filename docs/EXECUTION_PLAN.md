...
### 2026-07-22：hLF 湿实验周报对照与模型范围收敛

2026-07-13 收到一份 hLF 湿实验进度周报（周报级别、**不触发方向 1 的真实数据回填**）。**保密约束（用户 2026-07-22 要求）：具体基因位点/菌株基因型/构建/产量数值不写入文档、不上传，此处只留机制层结论。** **模型范围内的两条相对判断被验证**：在已增强折叠能力与 UPR 的本底菌株上，进一步增强折叠的收益已饱和（印证 R2 saturating、“推开后瓶颈转移”）；某个内质网→高尔基体运输因子过表达出现生长毒性（印证运输 OE 是弱/负信号）。**真实产量前沿全在模型盒子外**：产物降解（敲除蛋白酶）、转录调控/UPR、目标基因剂量、发酵工艺（温度/pH/补料，放大培养约一个数量级增益）、糖型结构（人源化改宿主糖基化通路、目标是糖链结构而非通量）。据此收敛三条：

1. **方向 4（组合改造）维持不做**，理由从“单基因分数不够可信”强化为“真实的组合改造在模型范围之外，在模型里搜遗传组合（含遗传算法/模拟退火/MILP）低价值”。
2. **糖基化改造在模型有用范围外**：模型只数糖基化通量/资源、不判糖链结构（人源化目标就是结构），相关敲除的细胞壁/生长风险也是结构性、模型看不见；仅更深度人源化（加非天然供体）时模型的“供体供给可行性”才有边际用处。
3. **新增候选方向：RNA-seq 表达约束的菌株特异建模**。将来会拿到 RNA-seq，其最高价值是用表达量约束酶容量（与蛋白约束模型天然契合），把相对层从“通用未标定”贴向“这一株特异”，触及核心未标定缺口 + 方向 1；它**不**解锁降解层建模。当前先做数据契约/方法准备，已写成 [ADR-005](adr/005-rnaseq-expression-constrained-enzyme-capacity.md)（契约与方法已定：pc 原生 transcript→酶丰度上界、经 curated 基因→复合体映射触达分泌 binding 层、绝对容量恒 unavailable；实现待数据），数据到位再落地。降解通路建模仍按原判断搁置。
...
- 仓库根 `tests/`：334 passed，0 failed（本轮 ①/R2 产品化 + 边界修复后）。
- `python_pichia/tests/`：本轮未改动引擎、未重跑；最近一次全量隔离 555 passed / 21 skipped / 0 failed（见 handoff）。
