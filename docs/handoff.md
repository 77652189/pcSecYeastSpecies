...
已完成（阶段④）：E0 易用性（筛查页可读基因名 + KO 可视化 + 「基因扰动」层级重排，`23aaeaf`→`dc71478`）、E1 候选面板诚实化（`389721f`）。
下一步：**E2 统一候选选择器**（基因与复合体并列、按分泌环节分组、勾选即加入 KO/OE，四个输入框降为高级入口）→ **E3 基因↔复合体映射软件侧**（ADR-007 数据契约 + 门禁，策展范围与责任人待拍板）。
