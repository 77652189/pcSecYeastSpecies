...
**E3 基因↔复合体映射 · 软件侧已完成**（引擎 `gene_complex_mapping` + 服务门面 + 选择器「实验时对应基因」列；门禁：未复核 / 计量未知 / 非必需亚基一律不得用于"单基因 OE 提升复合体容量"声称；无策展数据优雅降级为 `—`）。

下一步：**策展数据到位**——把映射写进 `Data/pcSecPichia/gene_complex_mapping.json` 即生效、无需改代码。**待拍板：策展范围与责任人**（哪些复合体先做、谁判断映射可信；生物学判断，非软件问题）。阶段④ 软件侧至此收口。
