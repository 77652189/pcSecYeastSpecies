...
def render_shadow_cross_check() -> None:
    ...
    with run_tab:
        ...
        screen_run_id = st.text_input("screen_run_id", value="", key="shadow_lp_cross_check_screen_run_id")
        saved_result_path = st.text_input("saved_result_path", value="", key="shadow_lp_cross_check_saved_result_path")
        output_dir = st.text_input("output_dir", value="", key="shadow_lp_cross_check_output_dir")
    ...
    with manifest_tab:
        manifest_path = st.text_input("manifest_path", value="", key="shadow_lp_cross_check_manifest_path")
...
def _render_status(response: Mapping[str, Any]) -> None:
    if response.get("status") == "ok":
        ...
        st.warning(str(response.get("status", "review_required")))
...
def _render_last_response(response: Mapping[str, Any] | None) -> None:
    ...
    st.subheader("最近一次结果")
    cols = st.columns(3)
    cols[0].metric("status", str(response.get("status", "")))
    cols[1].metric("within_tolerance", str(response.get("within_tolerance", "")))
    cols[2].metric("relative_diff", _display_value(response.get("relative_diff")))
