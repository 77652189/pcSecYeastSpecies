...
def _validate_row_tokens_match_evidence(
    ...
    for token in set(re.findall(r"\b(?:PAS|AT)[A-Za-z0-9_\-]+\b", row_text)):
        if allowed_gene_ids and token not in allowed_gene_ids:
            blocking.append(
                {
    ...
    for token in set(re.findall(r"\bsec_[A-Za-z0-9_]+|\b[A-Z0-9]+_no_[0-9]+_(?:fwd|rvs)\b", row_text)):
        if allowed_reaction_ids and token not in allowed_reaction_ids:
            blocking.append(
                {
                    "type": "evidence_token_mismatch",
                    "location": location,
    ...
    for token in set(re.findall(r"\b(?:ui|overnight|phase|pilot|catalog|gene)[A-Za-z0-9_\-]*\b", row_text)):
        if token in {"gene", "catalog"}:
            continue
        if allowed_source_runs and token.startswith(("ui_", "overnight_", "phase", "pilot_")) and token not in allowed_source_runs:
            blocking.append(
                {
                    "type": "evidence_token_mismatch",
                    "location": location,
    ...
    for token in set(re.findall(r"\b\d+\.\d{3,}\b", row_text)):
